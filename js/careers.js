$(document).ready(function(){
  ...
  $('.js-careers').swipeleft(function(){ // 오른쪽에서 왼쪽으로 swipe(오른쪽으로 이동)
    ...
    if(career_pos == 0){ // 첫 번째 carousel__item에 위치했을 경우

      $(".js-carousel__list").css("margin-left", (move * -1)+"px"); // 두 번째 carousel__item으로 이동
      $('.js-carousel__indicator-item:nth-child(2)').addClass('active'); // 두 번째 indicator__item에 active 클래스 추가

    } else if(career_pos == move * -1) { // 두 번째 carousel__item에 위치했을 경우

      $(".js-carousel__list").css("margin-left", (move * -2)+"px"); // 세 번째 carousel__item으로 이동
      $('.js-carousel__indicator-item:nth-child(3)').addClass('active'); // 세 번째 indicator__item에 active 클래스 추가

    } else if(career_pos == move * -2) { // 세 번째 carousel__item에 위치했을 경우

      $(".js-carousel__list").css("margin-left", "0px"); // 첫 번째 carousel__item으로 이동
